 ...
 * Upload homework photo with OCR extraction
 */
...
export default function HomeworkUploadPage() {
  ...
  const uploadMutation = useMutation({
    mutationFn: ({ subjectId, title, photo }: { subjectId: number; title: string; photo: File }) =>
      homeworkAPI.upload(subjectId, title, photo, token!),
    onSuccess: () => {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Upload Homework</h1>
        <p className="text-gray-600 mt-2">
          Take a photo of your homework and AI will extract the text
        </p>
      </div>
      ...
      {uploadMutation.isError && (
        <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg text-red-700">
          <p className="font-medium">Failed to upload homework</p>
